...
bool mat3::ToEulerAngles(RotOrder order, vec3& angleRad) const
{
	...
	case YXZ:
		...
		if (angleRad[VX] > -M_PI_2 + EPSILON)
		{
			...
			{
				angleRad[VZ] = 0.0f;
				angleRad[VY] = atan2(mM[0][1], mM[0][2]);
				result = false;
			}
...
void quat::FromRotation(const mat3& rot)
{
	...
	//I tried using this function we used in class but it wasnt not rotating so when I saw the piazza post about the four cases I used that to learn more about those cases and
	//eventually referenced the orientation representation van vert and bishop supplmental bishop for the logic of the function as well as 
	//https://www.euclideanspace.com for information about the trace of a matrix as that was in the reading
	/*mQ[VW] = sqrt(rot[0][0] + rot[1][1] + rot[2][2] + 1) / 2;
	mQ[VX] = (rot[2][1] - rot[1][2]) / (4 * mQ[VW]);
	mQ[VY] = (rot[0][2] - rot[2][0]) / (4 * mQ[VW]);
	mQ[VZ] = (rot[1][0] - rot[0][1]) / (4 * mQ[VW]); */

	double mTrace = rot[0][0] + rot[1][1] + rot[2][2];

	if (mTrace > 0)
	{
		double S = sqrt(mTrace + 1);
		mQ[VW] = S * .5;
		mQ[VX] = (rot[2][1] - rot[1][2]) / (2 * S);
		mQ[VY] = (rot[0][2] - rot[2][0]) / (2 * S);
		mQ[VZ] = (rot[1][0] - rot[0][1]) / (2 * S);

	}
	//the previous method wont work if the trace is less than zero
	else if ((rot[0][0] > rot[1][1])& (rot[0][0] > rot[2][2])) {
		mQ[VX] = .5 *  sqrt(1.0 + rot[0][0] - rot[1][1] - rot[2][2]);
		mQ[VY] = (rot[0][1] + rot[1][0]) / (4 * mQ[VX]);
		mQ[VZ] = (rot[0][2] + rot[2][0]) / (4 * mQ[VX]);
		mQ[VW] = (rot[2][1] - rot[1][2]) / (4 * mQ[VX]);
	}
	else if (rot[1][1] > rot[2][2])
	{
		mQ[VY] = .5 *  sqrt(1.0 + rot[1][1] - rot[0][0] - rot[2][2]);
		mQ[VX] = (rot[0][1] + rot[1][0]) / (4 * mQ[VY]);
		mQ[VZ] = (rot[1][2] + rot[2][1]) / (4 * mQ[VY]);
		mQ[VW] = (rot[0][2] + rot[2][0]) /( 4 * mQ[VY]);

	}
	else {
		mQ[VZ] = .5 *  sqrt(1.0 + rot[2][2] - rot[0][0] - rot[1][1]);
		mQ[VX] = (rot[0][2] + rot[2][0]) / (4 * mQ[VZ]);
		mQ[VY] = (rot[2][1] - rot[1][2]) / (4 * mQ[VZ]);
		mQ[VW] = (rot[0][1] + rot[1][0]) /  (4 * mQ[VZ]);
	}
...
quat quat::Slerp(const quat& q0, const quat& q1, double u)
{
	...
	double angle = acos(quat::Dot(q0, q1));
	if (angle == 0)
	{
		q;
	}
		...
		//q;
	    q = ((sin(1 - u) * angle) / sin(angle)) * q0 + (sin(angle * u) / sin(angle)) * q1;
	}
...
quat quat::SBisect(const quat& a, const quat& b)
{
	...
	quat c = a + b;
	q = (a + b) / sqrt(c[0] * c[0] + c[1] * c[1] + c[2] * c[2] + c[3] * c[3]);
	return q.Normalize();
}
...
quat quat::Scubic(const quat& b0, const quat& b1, const quat& b2, const quat& b3, double u)
{
	...
	b01 = quat::Slerp(b0, b1, u);
	b11 = quat::Slerp(b1, b2, u);
	b21 = quat::Slerp(b2, b3, u);
	b02 = quat::Slerp(b01, b11, u);
	b12 = quat::Slerp(b11, b21, u);
	b03 = quat::Slerp(b02, b12, u);

	result = b03;
	std::cout << "scubic" << std::endl;
...
void quat::ToAxisAngle (vec3& axis, double& angleRad) const
{
	...
	else
	{
		...
		axis[0] = mQ[VX] / (sin(angleRad / 2));
		axis[1] = mQ[VY] / (sin(angleRad / 2));
		axis[2] = mQ[VZ] / (sin(angleRad / 2)); 
	} 
...
mat3 quat::ToRotation () const
{
	...
	m[0][1] = (mQ[VZ] * mQ[VX] * mQ[VY] + (2 * mQ[VW] * mQ[VZ]));
